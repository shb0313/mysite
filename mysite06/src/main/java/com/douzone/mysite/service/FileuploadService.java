...
public class FileuploadService {
	...
	public String galleryStore(MultipartFile file) {
		String url = null;
		
		try {
			File uploadDirectory = new File(SAVE_PATH + "/gallery");
			if(!uploadDirectory.exists()) {
				uploadDirectory.mkdirs();
			}
			...
			byte[] data = file.getBytes();
			OutputStream os = new FileOutputStream(SAVE_PATH + "/gallery/" + saveFilename);
			os.write(data);
			os.close();
			
			url = URL_PATH + "/gallery/" + saveFilename; 
		} catch(IOException ex) {
			throw new FileuploadServiceException(ex.toString());
		}
